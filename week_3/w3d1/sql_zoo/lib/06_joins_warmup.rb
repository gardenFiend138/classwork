...
def casablanca_cast
  ...
  -- HAHA \/, going for a triple inner join when you don't even get joins
  -- SELECT
  --   name
  -- FROM
  --   movies
  -- INNER JOIN
  --   castings ON movies.id = castings.movie_id AS new_table
  -- INNER JOIN new_table ON new_table.actor_id =  actors.id
  -- WHERE
  --   id = 1;
  SQL
end
