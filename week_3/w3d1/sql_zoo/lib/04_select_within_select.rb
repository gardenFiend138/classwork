...
def population_constraint
  ...
  FROM
    countries
  WHERE
    name != 'Canada' AND name != 'Poland' AND population BETWEEN (
      SELECT
        population
      FROM
        countries
...
-- Other way to do it: instead of using BETWEEN, use greater than and less
-- then; less undoing.
  SQL
end
...
def sparse_continents
  ...
  execute(<<-SQL)
  -- The main query returns the name, continent, and population of those
  -- countries not in the subquery table; the subquery finds the continents
  -- that DO have countries with populations greater than 25M.
  SELECT
